-- Add industry and description columns to phrases table.
-- industry:    broad vertical (Healthcare, Insurance, etc.)
-- description: user-facing scenario description that rolls up into the general use_case
ALTER TABLE phrases ADD COLUMN IF NOT EXISTS industry text;
ALTER TABLE phrases ADD COLUMN IF NOT EXISTS description text;

-- Deactivate all previous phrases so only the new industry×use_case set is live
